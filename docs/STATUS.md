...
10. **`cp -r` bulk-write perf.** Currently ~1-2 files/sec on USB (≈ 30-40 min for the 22 k-file phone-backup workload, vs. Apple's driver's ~5-10 min). Three contributors. **(a)** ~~`refreshParentI30Size` deferral~~ ✅ Done (v0.4 Phase 1). `Volume.beginBulkInsert(into:)` / `endBulkInsert()` defer per-file size-hint refreshes; cp -r wraps each destination directory's file batch. Expected ~2× speedup. **(b) per-directory parent-record coalescing** — when cp -r adds N files to one dir, we currently re-read + re-write the parent MFT record N times; batch to one rewrite at end of dir. ~1 day, ~3-5× speedup on bulk inserts. v0.4 Phase 2. **(c) async write pipelining** — issue write N+1 while N is in flight. Bigger refactor (~3-5 days); v0.4 Phase 4, conditional on whether (a)+(b) are enough.
