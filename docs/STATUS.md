...
11. **`$INDEX_ROOT` multi-level split.** **Phase 3(A) ✅ Done.** When a leaf split's median promotion overflows the parent MFT record, `splitLeafAndPromote` allocates 2 intermediate INDX blocks, bisects `$INDEX_ROOT`'s interior entries into them, and replaces `$INDEX_ROOT` with a tiny new root. Tree depth: 2 → 3. Capacity: ~60 → ~75-150 files per dir on the small fixture; multi-thousand on real volumes once Phase 3(B) lands. **Phase 3(B) deferred.** Recursive interior-block split (when an intermediate INDX block itself fills) isn't implemented; clean `unsupportedFeature` abort until then. For workloads that need >150 files in one dir, Phase 3(B) is the follow-up.
