...
11. **`$INDEX_ROOT` multi-level split.** **Phase 3(A) ✅ Done.** When a leaf split's median promotion overflows the parent MFT record, `splitLeafAndPromote` allocates 2 intermediate INDX blocks, bisects `$INDEX_ROOT`'s interior entries into them, and replaces `$INDEX_ROOT` with a tiny new root. Tree depth: 2 → 3. **Phase 3(B) ✅ Done.** When the leaf's direct parent is an INTERMEDIATE INDX block (depth-3+ tree), the new median promotes into THAT intermediate via tree-walk + splice + rewrite. New capacity: ~3000-5000 files per dir on real volumes (small fixture caps at ~117 due to MFT slot constraints, not index algorithm). **Phase 3(C) deferred.** When the intermediate ITSELF fills, the proper fix is to split it (interior-block split) and cascade the median up to the grandparent (potentially all the way back to `$INDEX_ROOT`). Until then: clean `unsupportedFeature` abort. For workloads needing >5000 files in a single directory, Phase 3(C) is the follow-up — same compute-first transactional pattern, just one more level of recursion.
