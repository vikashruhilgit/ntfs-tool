...
11. **`$INDEX_ROOT` multi-level split.** **Phase 3(A) ✅ Done** ($INDEX_ROOT height-grow → tree depth 2→3). **Phase 3(B) ✅ Done** (promote median into intermediate INDX block, depth-3 fits case). **Phase 3(C) ✅ Done** (recursive interior-block split — when an intermediate overflows, bisect it + cascade median up to grandparent, loop until either a level fits OR we reach `$INDEX_ROOT`). Per-dir capacity: ~1077 files on the small fixture, **multi-thousand+ on real volumes** (limited only by free clusters + MFT slot count). **Phase 3(D) deferred.** When the cascade reaches `$INDEX_ROOT` AND the root itself can't fit the bubbled-up median on a depth-N tree, we'd need to height-grow on top of the cascade. Rare in practice (typically several thousand files per dir before this fires). Until then: clean `unsupportedFeature` abort. For workloads needing tens of thousands of files in one dir, Phase 3(D) is the follow-up.
