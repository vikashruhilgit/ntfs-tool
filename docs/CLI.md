...
| `find` | `<device> [path] --name <glob>` | Recursively search a directory and print the full path of every entry whose **name** matches the glob — `find -name` semantics (not `-path`), case-insensitive, which is the right default for NTFS. `--type f` restricts to files, `--type d` to directories; `--depth N` bounds the descent; default target = root (`/`), `--recnum` for the numeric form. Unreadable directories are reported on stderr and do not abort the search. Note the search root itself is never a candidate, and a target that resolves but cannot be walked (e.g. a file) reports on stderr and exits 0 — only an unresolvable target is a usage error (exit 64). **Read-only.** |
