...
public struct DirectoryWalker {
    ...
    /// `isLastSibling` is computed against the filtered listing.
    ///
    /// A DOS entry is dropped only when the same MFT record also carries a
    /// non-DOS name. A blanket `namespace != .dos` filter would make a file
    /// whose *only* `$FILE_NAME` is an 8.3 alias vanish from the walk
    /// entirely — rare on Windows-authored volumes, but a silent omission is
    /// the worst failure mode for `tree`/`find`, which callers read as a
    /// complete listing.
    public static func dropRedundantDOSAliases(_ entries: [DirectoryEntry]) -> [DirectoryEntry] {
        let recordsWithLongName = Set(
            entries.lazy.filter { $0.fileName.namespace != .dos }.map(\.recordNumber)
        )
        return entries.filter {
            $0.fileName.namespace != .dos || !recordsWithLongName.contains($0.recordNumber)
        }
    }

    private func enumerateFiltered(_ recordNumber: UInt64) async throws -> [DirectoryEntry] {
        Self.dropRedundantDOSAliases(try await volume.enumerate(directory: recordNumber))
    }
